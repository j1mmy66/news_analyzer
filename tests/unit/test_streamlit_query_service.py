...
def _news_hit(external_id: str, published_at: str, *, sort_published_at: object | None = None) -> dict[str, object]:
    return {
        "_id": external_id,
        "_source": {
        ...
        "sort": [sort_published_at if sort_published_at is not None else published_at, external_id],
    }


def test_latest_news_page_uses_stable_sort_cursor_and_48h_window() -> None:
        ...
        [
            {
                "hits": {
                    "hits": [
                        _news_hit("id-2", "2026-03-16T10:00:00+00:00", sort_published_at=1710583200000),
                        _news_hit("id-1", "2026-03-16T09:59:00+00:00", sort_published_at=1710583140000),
                    ]
                }
            }
        ]
    ...
    page = service.latest_news_page(size=1, cursor=NewsCursor(1710586800000, "id-3"), now=now)

    assert len(page.items) == 1
    assert page.has_more is True
    assert page.next_cursor == NewsCursor(1710583200000, "id-2")
    call_body = client.calls[0]["body"]
    assert call_body["size"] == 2
    assert call_body["sort"] == [{"published_at": {"order": "desc"}}, {"external_id": {"order": "asc"}}]
    assert call_body["search_after"] == [1710586800000, "id-3"]
    assert call_body["query"] == {
        "bool": {
            "must": [
                _canonical_clause(),
                {
                    "range": {
                        "published_at": {
                            "gte": "2026-03-15T08:00:00+00:00",
                            "lte": "2026-03-17T08:00:00+00:00",
                        }
                    }
                },
            ]
...
def test_latest_news_page_applies_source_and_class_filters() -> None:
    ...
    service.latest_news_page(size=50, source="rbc", class_label="economy", now=now)
...
def test_latest_news_page_applies_lenta_source_filter() -> None:
    ...
    service.latest_news_page(size=20, source="lenta", now=now)
...
def test_latest_news_page_disables_has_more_when_cursor_cannot_be_built(caplog) -> None:
    broken_hit = _news_hit("id-2", "2026-03-16T10:00:00+00:00")
    broken_hit.pop("sort")
    client = _FakeClient(
        [
            {
                "hits": {
                    "hits": [
                        broken_hit,
                        _news_hit("id-1", "2026-03-16T09:59:00+00:00"),
                    ]
                }
            }
        ]
    ...
    page = service.latest_news_page(size=1)

    assert len(page.items) == 1
    assert page.next_cursor is None
    assert page.has_more is False
    assert "Feed pagination cursor is missing or invalid" in caplog.text

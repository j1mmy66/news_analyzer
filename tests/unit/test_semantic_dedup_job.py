...
from news_analyzer.domain.models import DedupBulkUpdateResult, DedupMetadataUpdate
...
def test_run_semantic_dedup_job_updates_repository(monkeypatch) -> None:
    ...
    class _FakeRepository:
        ...
        def set_dedup_metadata_bulk(self, updates, *, updated_at):
            ...
            return DedupBulkUpdateResult(attempted=2, updated=2, failed_ids=[])
...
def test_run_semantic_dedup_job_returns_successful_updates_on_partial_write_failures(monkeypatch) -> None:
    class _FakeIndexManager:
        def __init__(self, client: object) -> None:
            return None
    ...
    class _FakeRepository:
        ...
        def set_dedup_metadata_bulk(self, updates, *, updated_at):
            ...
            assert isinstance(updated_at, datetime)
            return DedupBulkUpdateResult(attempted=2, updated=1, failed_ids=["id-2"])
...
def test_run_semantic_dedup_job_returns_zero_when_deduplicate_raises(monkeypatch) -> None:
    class _FakeIndexManager:
        def __init__(self, client: object) -> None:
            return None
    ...
    class _FakeRepository:
        ...
        def get_news_for_dedup_candidates(self, *, lookback_hours: int, limit: int):
            assert lookback_hours == 24
            assert limit == 50
            return [{"external_id": "id-1"}]

        def set_dedup_metadata_bulk(self, updates, *, updated_at):
            raise AssertionError("set_dedup_metadata_bulk should not be called")
    ...
    assert semantic_dedup_job.run_semantic_dedup_job(limit=50) == 0
